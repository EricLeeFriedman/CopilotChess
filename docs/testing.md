...
| `src/ui_tests.cpp` | Tests for the UI subsystem: `DrawBoard` does not crash on a freshly initialized `GameState`; the center of a selected square has a non-background color; a legal-move dot appears only on the correct target square |
