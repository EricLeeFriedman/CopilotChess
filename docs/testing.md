...
| `src/ui_tests.cpp` | Tests for the UI subsystem: `DrawBoard` does not crash on a freshly initialized `GameState`; the center of a selected empty square is exactly `BOARD_SELECTED` (positive equality assertion); the center of an empty legal-move target is exactly `BOARD_MOVE_DOT` (positive equality assertion), and a non-target square retains its plain board color; the top-left corner of the king's square is exactly `BOARD_CHECK` when White is in check, and separately when Black is in check; the king square retains `BOARD_DARK` (not `BOARD_CHECK`) when the side to move is safe; after Fool's Mate `DrawStatusOverlay` paints a dark banner at the expected location (positive `STATUS_BANNER_BG` assertion); the same banner appears after a stalemate position; and `DrawStatusOverlay` is a no-op (banner location retains the clear colour) when the game is ongoing |
