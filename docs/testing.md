...
| `src/ui_tests.cpp` | Tests for the UI subsystem: `DrawBoard` does not crash on a freshly initialized `GameState`; the center of a selected empty square is exactly `BOARD_SELECTED` (positive equality assertion); the center of an empty legal-move target is exactly `BOARD_MOVE_DOT` (positive equality assertion), and a non-target square retains its plain board color; the top-left corner of the king's square is exactly `BOARD_CHECK` when White is in check, and separately when Black is in check; the king square retains `BOARD_DARK` (not `BOARD_CHECK`) when the side to move is safe; after Fool's Mate `DrawStatusOverlay` paints a dark banner at the expected location (positive `STATUS_BANNER_BG` assertion) **and** a text pixel from the 'B' glyph in "BLACK WINS" (`STATUS_TEXT_COLOR`), proving the Black-win message branch and glyph rendering; after a stalemate position `DrawStatusOverlay` paints the banner **and** a text pixel from the 'S' glyph in "STALEMATE" (`STATUS_TEXT_COLOR`), proving the draw branch; `DrawStatusOverlay` is a no-op when the game is ongoing; after a White-win checkmate position `DrawStatusOverlay` paints both the banner background and a specific text glyph pixel, proving the "WHITE WINS" message branch; **Both-color and draw coverage for `DrawGameOverOverlay`**: `GAME_WHITE_WINS` asserts `RESULT_WHITE_COL` on the result strip and `BTN_FILL` on the restart button interior; `GAME_BLACK_WINS` asserts `RESULT_BLACK_COL` on the strip, a near-white text pixel from 'C' in "CHECKMATE - BLACK WINS", and `BTN_FILL`; `GAME_DRAW` asserts `RESULT_DRAW_COL` on the strip, a near-white text pixel from 'S' in "STALEMATE - DRAW", and `PIECE_W_FILL` at the white-king body interior proving the both-king rendering path; `IsRestartButtonHit` returns `true` for pixels inside the restart button (centre, top-left corner, bottom-right corner) and `false` for pixels outside (one pixel above, below, left, right); **Turn indicator coverage**: `DrawTurnIndicator(COLOR_WHITE)` asserts `STATUS_BANNER_BG` on the banner background, a text pixel from the 'W'-only column (col 4 of row 0, bit pattern `0b10001`) set to `STATUS_TEXT_COLOR`, and a 'W'-blank column that would be set by 'B' (`0b11110`, col 1) is not text-color, proving the "WHITE TO MOVE" branch renders the correct glyph; `DrawTurnIndicator(COLOR_BLACK)` mirrors this with a 'B'-only pixel set and the 'W'-only pixel absent, proving the "BLACK TO MOVE" branch |
