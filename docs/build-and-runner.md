...
| PR Adversarial Review | `.github/workflows/pr-review.yml` | `ubuntu-latest` | Installs GitHub Copilot CLI, classifies the PR by changed-file type, and invokes the `cpp-pr-review` custom agent only for code PRs (changes to `src/` or `build.ps1`). Pure process/docs/workflow PRs (only `docs/`, `.github/`, `AGENTS.md`, `README.md`, or other non-code files) skip the `cpp-pr-review` agent and receive a lightweight COMMENT acknowledgment instead. Mixed PRs (code + docs) are treated as code PRs. The Copilot agent step requires either a `COPILOT_REVIEW_TOKEN` or `PERSONAL_ACCESS_TOKEN` repository secret containing a fine-grained PAT with `Copilot Requests` permission. The workflow grants `pull-requests: write` and `issues: write` to `GITHUB_TOKEN` so both the review submission and the PR issue-comment steps always succeed without a PAT. The agent returns a JSON object with a `blocking` array (required fixes) and a `comments` array (optional observations). The workflow posts `REQUEST_CHANGES` whenever `blocking` is non-empty, regardless of the agent's top-level `event` field; an `event=COMMENT` with non-empty `blocking` is coerced to `REQUEST_CHANGES` and the mismatch is logged. Parse failures are also posted as `REQUEST_CHANGES` instead of silently passing as `COMMENT`. When the review is `REQUEST_CHANGES`, a separate "Ping Copilot" step posts a PR comment mentioning `@copilot` using `PERSONAL_ACCESS_TOKEN || GITHUB_TOKEN`. The workflow enforces a hard limit of 7 `REQUEST_CHANGES` automated review rounds per pull request, counting only reviews authored by `github-actions[bot]`; once that limit is reached it posts a human-intervention notice via `GITHUB_TOKEN` and skips further automated reviews. |
| Validate PR Review Logic | `.github/workflows/validate-pr-review.yml` | `ubuntu-latest` | Runs `.github/scripts/validate-pr-review.py` to validate the `parse_review` JSON-parsing function and the `classify_pr` PR-type routing logic offline, without requiring a live pull request or Copilot CLI. Triggered on push and pull requests to `main` when `pr-review.yml` or `validate-pr-review.py` change. Exit code 0 means all tests passed. |
