...
| PR Adversarial Review | `.github/workflows/pr-review.yml` | `ubuntu-latest` | Installs GitHub Copilot CLI, invokes the repository's `cpp-pr-review` custom agent against the PR diff, and posts the resulting GitHub review for same-repository pull requests. The Copilot agent step requires either a `COPILOT_REVIEW_TOKEN` or `PERSONAL_ACCESS_TOKEN` repository secret containing a fine-grained PAT with `Copilot Requests` permission. The review submission step uses `GITHUB_TOKEN` so the structured machine review is always submitted successfully. When the review requests changes (`REQUEST_CHANGES`), a separate "Ping Copilot" step posts a PR comment via `PERSONAL_ACCESS_TOKEN` mentioning `@copilot` to trigger the coding agent; this step requires `PERSONAL_ACCESS_TOKEN` to have `issues: write` (fine-grained) or `repo` (classic) scope and only runs when the automated review has blocking findings. The workflow enforces a hard limit of 7 `REQUEST_CHANGES` automated review rounds per pull request; once that limit is reached it posts a human-intervention notice via `PERSONAL_ACCESS_TOKEN` and skips further automated reviews. |
