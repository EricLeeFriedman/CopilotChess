...
- When a review requests changes, a separate "Ping Copilot" step posts a PR comment via `PERSONAL_ACCESS_TOKEN` mentioning `@copilot` to trigger the coding agent for follow-up work. This step only fires when the automated review has blocking findings (`REQUEST_CHANGES`) and requires `PERSONAL_ACCESS_TOKEN` to have `issues: write` (fine-grained) or `repo` (classic) scope.
- The workflow enforces a hard cap of **7 blocking (`REQUEST_CHANGES`) automated review rounds** per pull request. Clean COMMENT reviews do not count toward the cap. Once 7 blocking reviews have been posted, the workflow stops and posts a comment via `PERSONAL_ACCESS_TOKEN` stating that human intervention is required.
